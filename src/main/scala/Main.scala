...
object Main extends App {
  ...
  def koodaa(z: Int, x: Int, y: Int, asento: Int): Char = {
    (z * 2048 + (x + 6) * 64 + (y + 6) * 2 + asento).toChar
  }

  def dekoodaa(pala: Char): (Int, Int, Int, Int) = {
    val num = pala.toInt
    val z = num / 2048: Int
    val x = ((num % 2048) / 64: Int) - 6
    val y = ((num % 64) / 2: Int) - 6
    (z, x, y, num % 2)
  }

  // Apufunktioita kombojen käsittelyyn

  def haeMinimit(kombo: String): (Int, Int, Int) = {
  ...
  def haeMitat(kombo: String, leveys: Int, korkeus: Int): (Int, Int, Int) = {
    ...
    (mittaZ, mittaX, mittaY)
  }

  def normalisoi(kombo: String): Unit = {
    ...
    if (kierros == 1) {
      ...
      while (jaljella > 0) {
        ...
        for (kombo <- edellisetKombot) {
          ...
          laskuri += 1
          if (laskuri % 100 == 0 || laskuri == maara) {
